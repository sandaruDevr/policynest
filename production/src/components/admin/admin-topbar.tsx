...
export function AdminTopbar({ context, onOpenMobileNav }: AdminTopbarProps) {
  const { profile, tenant, sites } = context;
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <div className="leading-tight">
            ...
            <p className="text-[11px] text-ink-muted">
              ...
              {tenant.stateOrTerritory ? ` · ${tenant.stateOrTerritory}` : ""}
            </p>
          </div>
        </div>
...
function AdminProfileMenu({
  ...
  const initials =
    (parts[0]?.charAt(0) || "A") + (parts.slice(-1)[0]?.charAt(0) || "");
      ...
      <DropdownMenuContent align="end" className="w-60">
        <DropdownMenuLabel className="!normal-case !tracking-normal">
          <div className="px-1 pb-1">
            ...
            <p className="text-xs text-ink-muted">{tenantName}</p>
          </div>
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
